...
pub(crate) fn provision_ssh(
    ...
    let authorized_keys_path_string = authorized_keys_path_string
        .unwrap_or_else(|| {
            get_authorized_keys_path_from_sshd(get_sshd_g_output)
                .unwrap_or_else(|| ".ssh/authorized_keys".to_string())
        });
    let authorized_keys_path = user.dir.join(authorized_keys_path_string);
...
fn get_sshd_g_output() -> io::Result<Output> {
...
fn get_authorized_keys_path_from_sshd(
    ...
    let output: Output = match run_sshd_command(sshd_config_command_runner) {
        Some(output) => output,
        None => {
            info!("Unable to determine authorizedkeysfile from sshd -G");
            return None;
        }
    };

    match extract_authorized_keys_file_path(&output.stdout) {
        Some(path) => Some(path),
        None => {
            info!("No authorizedkeysfile in sshd configuration");
            None
        }
    }
}
...
fn run_sshd_command(
    ...
    match sshd_config_command_runner() {
        Ok(output) if output.status.success() => {
            info!(
                "sshd -G succeeded with stdout length={:?}",
                output.stdout.len()
            );
            Some(output)
        }
        Ok(output) => {
            error!(
                "sshd -G failed with code={} stdout={:?} stderr={:?}",
                output.status,
        ...
        Err(e) => {
            error!("Failed to execute sshd -G due to {}", e);
            None
        }
    }
}
...
fn extract_authorized_keys_file_path(stdout: &[u8]) -> Option<String> {
    ...
    for line in output.lines() {
        if line.starts_with("authorizedkeysfile") {
            ...
            if keypath.len() > 1 {
                info!("Found authorizedkeysfile path: {}", keypath[1]);
                return Some(String::from(keypath[1]));
            }
        }
    }
    None
