...
impl SysOps for OsSysOps {
    ...
    fn boot_time(&self) -> io::Result<i64> {
        parse_proc_stat_btime(&std::fs::read_to_string(
            &self.proc_stat_path,
        )?)
    }
}
...
impl KvpPoolIter {
    ...
    fn remove_current(&mut self) -> io::Result<()> {
        ...
        self.handle.set_len(last_index as u64 * RECORD_SIZE as u64)?;
        self.record_count -= 1;
        Ok(())
    }
}
...
mod tests {
    ...
    impl Read for MemHandle {
        fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
            ...
            let data =
                g.files.get(&self.path).cloned().unwrap_or_default();
    ...
    fn test_mock_seek_invalid_negative_position() {
        ...
        let mut h: Box<dyn Handle> =
            ops.open_read_write_create(&path).unwrap();
    ...
    fn test_mock_path_metadata_defaults_mtime_when_absent() {
        ...
        ops.0.lock().unwrap().files.insert(path.clone(), vec![0u8; 4]);
